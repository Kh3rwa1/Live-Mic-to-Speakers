...
- **Paced burst loop:** Live monitoring streams audio in exact 1-burst chunks (`framesPerBuffer`, typically 96–192 frames = 2–4 ms @ 48 kHz). By avoiding multi-burst buffer reads, input queuing delay is reduced by up to 10x before audio reaches the gain stage.
- **Adaptive output buffer tuning (`OutputBufferTuner`):** While `AudioTrack` buffer capacity satisfies platform `getMinBufferSize()` to guarantee successful device creation, the active playback buffer is dialed down to 2 bursts via `setBufferSizeInFrames()` on API 24+ (e.g. 384 frames / ~8 ms @ 48 kHz). If buffer underruns are detected, buffer size automatically expands by 1 burst (up to 4 bursts or capacity, with a 250 ms cooldown) and never shrinks below 2 bursts.
- **Clock drift mitigation (`DriftController`):** Discrepancies between physical input and output crystal clocks cause playback queue drift over long sessions. The controller tracks instantaneous queue depth (`framesWritten - playbackHeadPosition`). If the queue remains continuously above the target depth for over 1.0 second, a smooth crossfade correction (dropping at most 1 burst, &le; 2 ms) eliminates excess latency without audible clicks or pops, rate-limited to at most once per 250 ms.
...
- **Input profiles:**
  - *Low latency (Recommended):* Selects `VOICE_PERFORMANCE` (API 29+) or `VOICE_RECOGNITION` (API 24–28) with `AudioAttributes.USAGE_MEDIA` and `CONTENT_TYPE_SPEECH`. Bypasses hardware echo cancellation and noise suppression filters for minimum roundtrip delay (~15–35 ms). Ideal for headphones and external PA speakers.
  ...
  - *Noisy room / Call-style:* Selects `VOICE_COMMUNICATION` with Acoustic Echo Canceler (AEC) and Noise Suppressor (NS) active, matching `USAGE_VOICE_COMMUNICATION`. Maximizes echo prevention in echo-prone rooms at the cost of telephony DSP latency (~40–80 ms).
- **Bluetooth honesty:** Real-time route classification warns users with a non-blocking card when wireless Bluetooth audio (`TYPE_BLUETOOTH_A2DP`, `TYPE_BLUETOOTH_SCO`, `TYPE_BLE_*`) is active, explaining that Bluetooth's 100–250 ms delay is an inherent wireless protocol constraint and recommending wired headphones for the lowest latency.
...
- **Why Java path & expected latency floor:** Utilizing standard Android Java APIs (`AudioRecord` and `AudioTrack`) with the platform fast-track mixer (`PERFORMANCE_MODE_LOW_LATENCY`, marked with "F" in `dumpsys media.audio_flinger`) achieves roundtrip latency between 15–25 ms on modern flagship hardware and 25–45 ms on mid-range devices. This baseline demonstrates the performance limit of the Java framework prior to any potential native NDK/Oboe migration.
