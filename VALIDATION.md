...
- **Clock drift mitigation:** `DriftController` monitors input backlog (unread frames vs. elapsed session time or hardware timestamps). When input backlog stays continuously above burst + 1 burst of margin for > 1.0 second, a smooth crossfade correction drops up to 1 burst (&le; 2 ms) without audible clicks, rate-limited to at most once per 250 ms.
