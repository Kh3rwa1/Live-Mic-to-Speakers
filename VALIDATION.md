...
Use the checks on the exact commit you plan to merge or release. Current evidence is recorded in [PR #6](https://github.com/Kh3rwa1/Live-Mic-to-Speakers/pull/6); a workflow definition is not a passing result.
...
- **Actual keyboard behavior:** an instrumented test opens the software keyboard in the app window and requires an editable control to remain visible above it. This runs on APIs 24/34/36, including API 36 at 200% font scale; no API is excluded to make the suite pass.
...
- **CI regressions:** nine runner tests and seventeen native-validator tests supplement the existing app/ads JVM, lint and Android suites. Demo-mode and missing-production-setting rejection are checked using separate logs so one test cannot accidentally satisfy the other's assertion.

App build and device-test jobs have read-only repository permissions. An isolated, same-repository PR reporter can read Actions logs and write concise failure comments; it never checks out or executes application code.

## Where to find the evidence

In the successful Actions run, download:

- `live-mic-debug-apk` for installation on a test device.
- `android-test-and-lint-reports` for unit/lint results, production-gate logs, `native-alignment.json` and ZIP-alignment output.
- `android-device-reports-api-<API>-font-<scale>` for instrumentation reports and the native screenshots.

Run CI helper checks locally with `python3 -m unittest discover -s tools/tests -v`.
...
2. **Visual/accessibility:** inspect the collected native images and test TalkBack, keyboard focus, large text, RTL, cutouts and resizing. Bounds assertions and screenshot capture are not visual or screen-reader sign-off. Library/history screens still need the wider design/localization audit.
...
No signing secrets, production ad identifiers, user recordings, repository protection rules, merges or store releases are changed by this pass.
