...
- **Low-latency audio pipeline & adaptive buffer tuning:** Live monitoring reads and writes in 1-burst chunks (`framesPerBuffer`), minimizing input buffer queuing delay (see [DEVICE_TEST_PLAN.md](docs/DEVICE_TEST_PLAN.md) results). `OutputBufferTuner` tunes `AudioTrack.setBufferSizeInFrames()` to 2 bursts while maintaining capacity for platform initialization, adaptively growing by 1 burst on underruns up to a 4-burst ceiling.
- **Clock drift mitigation:** `DriftController` monitors input backlog (unread frames vs. elapsed recording time or hardware timestamps). After the gain ramp, it captures a ~2-second median baseline. When input backlog stays continuously above baseline + 1 burst of margin for > 1.0 second, a smooth crossfade correction drops up to 1 burst (&le; 2 ms) without audible clicks, rate-limited to at most once per 250 ms.
...
- **Input profiles & AudioAttributes:** Three accessible input profiles in Settings (Low latency, Balanced, Noisy room) dynamically configure hardware AEC, NS, and candidate sources (`VOICE_PERFORMANCE`, `VOICE_RECOGNITION`, `MIC`), using `USAGE_MEDIA` across all profiles to ensure loudspeaker routing.
