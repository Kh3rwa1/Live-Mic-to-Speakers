...
- **CI regressions:** eleven dependency-policy tests, a failure-output regression and seven SDK-bootstrap tests supplement the existing nine runner and seventeen native-validator tests and the app/ads JVM, lint and Android suites. Demo-mode and missing-production-setting rejection use separate logs, with unexpected failures surfaced rather than hidden. Generated production resources use AGP's variant API; the production gate remains mandatory.
